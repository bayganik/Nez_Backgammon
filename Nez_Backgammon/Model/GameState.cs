...
namespace Backgammon.Model
{
    public class GameState
    {
        ...
        public GameState(BKBoard currentBoard)
        {
            ...
            {
                board[i] = currentBoard.GetNumOfCheckersInPipe(i);
            }
        }
        ...
        { // Huristic
            ...
            int pipeContainingMoreThanOneRedChecker = 0; // how many pipe contain
                                                         // (red checkers more
                                                         // than 1) negetive
                                                         // value
            ...
            int totalValueOfRedCheckers = 0; // sum of total number of red
                                             // checkers
            int totalValueOfWhiteCheckers = 0; // sum of total number of white
                                               // checkers

            int distanceForRedCheckers = 0; // distance (max distance : red checker)
                                            // if white checker in gyard then 25
            ...
            for (int b = 0; b < 24; b++)
            {
                ...
                else
                {
                    if (this.board[b] < -1)
                    {
                        pipeContainingMoreThanOneRedChecker++;
                    }
                    ...
                    if (tmp > distanceForRedCheckers)
                    {
                        distanceForRedCheckers = tmp;
                    }
                    totalValueOfRedCheckers += this.board[b];
                }
            }
            if (player == 1)
            {
                eval = (-0.025 * (pipeContainingMoreThanOneWhiteChecker - pipeContainingMoreThanOneRedChecker))
                     - (0.02525 * (Math.Abs(board[this.gyard(2)]) - board[this.gyard(1)]))
                     + 0.4 * totalValueOfWhiteCheckers + 0.0125 * distanceForWhiteChekers;
            }
            else
            { // computer
                eval = (0.025 * (pipeContainingMoreThanOneRedChecker - pipeContainingMoreThanOneWhiteChecker)) + (0.02525 * (board[this.gyard(1)] - Math.Abs(board[this.gyard(2)]))) - 0.4 * totalValueOfRedCheckers - 0.0125 * distanceForRedCheckers;

                // --------------------------------------------------

                //			eval = (0.025 * (pipeContainingMoreThanOneRedChecker - pipeContainingMoreThanOneWhiteChecker))
                //					+ (0.02525 * (board[this.gyard(1)] - Math.abs(board[this.gyard(2)])))
                //					- 0.4 * (Math.abs(totatotalValueOfRedCheckers) - totalValueOfWhiteCheckers)
                //					- 0.0125 * (distanceForRedCheckers - distanceForWhiteChekers);
            }
            return eval;
        }
        ...
        public bool hasGyard(int[] board, int player)
        {
        ...
        public static string print(int[] board2)
        {
            ...
            str += "[24]\t[25]\r\n";
            str += ((board2[24] == 0) ? "empty" : board2[24]) + "\t"
                    + ((board2[25] == 0 ? "empty" : board2[25])) + "\r\n";
            return str;
        }
